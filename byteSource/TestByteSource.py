...
class ByteSourceTestCase(unittest.TestCase):
    ...
    def assertErrorIsAllowed(self, expected, actual, allow_error):
        ...
        expected = np.where(np.array(expected) == 0, np.spacing(0), np.array(expected))
    ...
    def test_gen_msg_arr(self):
        ...
        :return: None
        """
        def test_gen_msg_arr_flow(prob, msg_size, allow_prob_error, allow_entropy_error):
            """
            测试仿真256元DMS工作流
            :param prob: 理论概率数组
            :param msg_size: 信息长度
        ...
        size = 10240000
        prob_error, entropy_error = 0.1, 0.001
        test_gen_msg_arr_flow(expected_prob, size, prob_error, entropy_error)
    ...
    def test_generate_bDMS_extended_source(self):
        """
        二元DMS扩展为8次信源工作流的单元测试
        :return: None
        """
        def test_flow(p_true, path, prob, msg_size, allow_prob_error, allow_entropy_error):
            """
