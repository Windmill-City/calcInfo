...
class ByteSourceTestCase(unittest.TestCase):
    ...
    def test_gen_msg_arr(self):
        ...
        def gen_msg_arr_flow(prob, entropy, msg_size, allow_prob_error, allow_entropy_error):
        ...
        prob_error, entropy_error = 0.20, 0.001
        gen_msg_arr_flow(expected_prob, expected_entropy, size, prob_error, entropy_error)

        size = 1024000
        prob_error, entropy_error = 0.15, 0.0001
        gen_msg_arr_flow(expected_prob, expected_entropy, size, prob_error, entropy_error)

        size = 10240000
        prob_error, entropy_error = 0.10, 0.00001
        gen_msg_arr_flow(expected_prob, expected_entropy, size, prob_error, entropy_error)

        expected_prob, expected_entropy, size = np.array([0.5 if n <= 1 else 0 for n in range(256)]), 1, 102400
        prob_error, entropy_error = 0.01, 0.00001
        gen_msg_arr_flow(expected_prob, expected_entropy, size, prob_error, entropy_error)
